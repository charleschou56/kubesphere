...
import (
	...
	cnatinformers "kubesphere.io/kubesphere/pkg/client/informers/externalversions/cnat/v1alpha1"
	cnatlisters "kubesphere.io/kubesphere/pkg/client/listers/cnat/v1alpha1"
...
const controllerName = "cnat-controller"
...
func (c *Controller) syncHandler(key string) (time.Duration, error) {
	...
	if !reflect.DeepEqual(original, instance) {
		...
		_, err = c.ksClient.CnatV1alpha1().Ats(instance.Namespace).Update(context.TODO(), instance, metav1.UpdateOptions{})
		if err != nil {
			return time.Duration(0), err
		}
	}
